...
if UTCDateTime(startTime) + float(run["time_interval"][0]) <= UTCDateTime(
    startTime[0:10] + "T23:59:59.999999Z"
):
    # If the end time of the data requested (startTime + time_interval given in the run param file) comes BEFORE midnight of the day of the data request then set endTime to startTime + time_interval. I.e., Only request data up until midnight of the date of the request. TODO: Figure out if the data requests are restricted to before midnight to prevent overlapping data requests. Modify the code if this is unnecessary. MT
    endTime = str(UTCDateTime(startTime) + float(run["time_interval"][0])) # End time of data request. Time_interval = duration of data request in seconds given in the run param file. Currently set to 3600 seconds (1 hour).
    print("END TIME:", endTime)

else:

    # If the end time of the data requested (startTime + time_interval given in the run param file) comes AFTER midnight of the day of the data request then set, set the endTime to just before midnight. I.e., Only request data up until midight of the date of the request. TODO: Figure out if the data requests are restricted to before midnight to prevent overlapping data requests. Modify the code if this is unnecessary. MT
    endTime = str(UTCDateTime(startTime[0:10] + "T23:59:59.999999Z"))
    print("END TIME:", endTime)
...
if (
    ...
    # If the data_endtime given for the data request in the run param file is "Now" or a future date, then set the truncTime to Now - trunctime (currently set to 24 hours in the run param file). I.e,. Only request data that is at least 24 hours old. TODO: Find out of if this is necessary and modify code if it is not. MT
    print(
...
# If the endTime (end time of data request) is less than or equal to the truncTime (either 24 hours before now or farther in the past) and the difTime (endTime - startTime) is greater than or equal to the diftime in the run param file (currently 5 seconds), then make data request. MT
if UTCDateTime(endTime) <= UTCDateTime(truncTime) and difTime >= float(
    ...
    count = 0
    maxCycle = int(run["maxCycle"][0]) # currently 5
    delay = int(
        run["delay"][0]
    ...
    if not success:
        ...
        ) # Old message implied that the request was sent 5 times and failed each time, which is inaccurate. MT
        print(
            "[FAILED]! COUNTS =",
            count,
            "Data request still incomplete after 5 status requests",
            "Response codes:",
            response_codes,
        ) # Old message implied that the request was sent 5 times and failed each time, which is inaccurate. MT
        # sendmail(
        #     "OOI WARNING: Not able to request data more than 5 times: "
        #     + reference_name_dash
        #     + "-"
        #     + run_name,
        #     "Status URL [" + statusUrl + "] got response codes " + response_codes,
        ...
        #     "[FAILED]! COUNTS =",
        #     count,
        #     "Request data more than 5 times",
        #     "Response codes:",
        #     response_codes,
        # )
    ...
    if complete == "complete":
        ...
        )  # Updated the HTTPSever_server and OPENDAP_server urls in the run param file , MT
        ...
        try:
            fh = Dataset(netcdfUrl) 
            t = fh.variables[str(run["datatime"][0])][:] # datatime = time TODO: What are the units of the time stamps?
        ...
        if len(t) >= int(run["data_sp"][0]):
            ...
            sp = round(np.median(np.diff(t)), 2) # Fix mentioned above. 
        ...
        # If there are fewer than data_sp (currently 5) samples in the netCDF file, then do not create miniSEED file. 
            ...
            print(
                ...
                + str(UTCDateTime(startTime) + float(run["time_interval"][0]))
                + ": ",
                "DATA POINT is less than " + run["data_sp"][0] + "! SKIP IT!",
            )
        ...
        # If number of samples is less than data_sp (currently 5), then do not create miniSEED file.
        ...
        for p in range(len(new_t)):
            ...
            new_utc_t.append(str(UTCDateTime(str(utcdata1900(float(new_t[p])))))) # Convert elements of new_t to UTC and save to list.

            # Manoch 2017-09-19: We should exclude the first sample since p-1 returns the last sample. Original logic works too since we always should have difference between the first and last sample larger than the sampling, but this should be a more stable logic

            # 2017-12-21 - change to fix array ref bug:
            if p == 0:

                statment.append(False)

            else:
                # Check if the rounded difference between new_t[p] and new_t[p-1] differs from sp (Median difference in time between values of t).
                statment.append(
                    round(np.diff(new_t)[p - 1], int(run["round_digit"][0])) != sp
                ) 

            print(">>>", np.where(statment)[0])
            data_split = np.split(new_utc_t, np.where(statment)[0]) # Split new_utc_t wherever the rounded difference between new_utc_t[p] and new_utc_t[p-1] differs from sp (Median difference in time between values of t).

        # Check if:
        # - The file_end_time (startTime - 0.1 s) is not right before midnight 
        # - The time of the second sample is not the same as the time of the first split in new_utc_t
        # - The difference between the time of the first split in new_utc_t and the time of the second sample is LESS than sp (median difference in time between values of t) # TODO: Review.
        if (
            str(file_end_time)[11:27] != "23:59:59.999999Z"
            and file_end_time + sp != UTCDateTime(data_split[0][0])
            and UTCDateTime(data_split[0][0]) - (file_end_time + sp) < sp
        ):
            ...
            if int(run["gap"][0]) == 1:
                ...
                with open(os.path.join(runPath, gap_file_name), "a") as output:
                    output.write(
                        "%s %s\n" % (str(file_end_time + sp), str(file_end_time + sp))
                    )
                    
        # Check if:
        # - The file_end_time (startTime - 0.1 s) is not right before midnight
        # - The time of the second sample is not the same as the time of the first split in new_utc_t
        # - The difference between the time of the first split in new_utc_t and the time of the second sample is GREATER THAN OR EQUAL TO sp (median difference in time between values of t) # TODO: Review
        elif (
            str(file_end_time)[11:27] != "23:59:59.999999Z"
            and file_end_time + sp != UTCDateTime(data_split[0][0])
            and UTCDateTime(data_split[0][0]) - (file_end_time + sp) >= sp
        ...
        # Check if:
        # - The file_end_time (startTime - 0.1 s) is not right before midnight 
        # - The requested_start (UTCDateTime(file_end_time + 0.10000 s) is not the same as the time of the first split in new_utc_t
        # - The difference between the time of the first split in new_utc_t and the time of the second sample is LESS than sp (median difference in time between values of t) # TODO: Review
        elif (
            str(file_end_time)[11:27] == "23:59:59.999999Z"
            and requested_start != UTCDateTime(data_split[0][0])
            and UTCDateTime(data_split[0][0]) - requested_start < sp
        ):
            # Gap is less than sp 
        ...
        # - The difference between the time of the first split in new_utc_t and the time of the second sample is GREATER THAN OR EQUAL TO sp (median difference in time between values of t) # TODO: Review
            ...
            str(file_end_time)[11:27] == "23:59:59.999999Z"
            and requested_start != UTCDateTime(data_split[0][0]) # requested start = file_end_time + 0.1 s
            and UTCDateTime(data_split[0][0]) - requested_start >= sp
        ):

            # Gap is greater than sp
            print(
                startTime + "  " + str(UTCDateTime(data_split[0][0]) - sp) + ": ", "GAP"
            )
            ...
            if int(run["gap"][0]) == 1:
                ...
                with open(os.path.join(runPath, gap_file_name), "a") as output:
                    output.write(
                        "%s %s\n" % (startTime, str(UTCDateTime(data_split[0][0]) - sp))
                    )
            ...
            for j in range(len(data_split)):
                ...
                if j == len(data_split) - 1:
                    ...
                    if len(ti) >= int(run["data_sp"][0]):
                        ...
                        # Add end_time (ti[-1]) + sp, start_time2 (ti2[0]) - sp to the gap file
                        if int(run["gap"][0]) == 1:
        ...
        for k in range(len(channel_ls)):
            ...
            data_point = fh.variables[name][:] * float(run["unit_convert"][0]) # Unit_convert currently set to 1, so this doesn't do anything. Comment in old run param file says that the unit conversion is now done via the metadata sensitivity value but I don't see where. MT TODO: If data is not converted here, when does it happen?
...
# If the endTime is more recent than 24 hours before the current time, skip the request. 
