...
# Original author: Vivian Teng
# Modified by Manoch (mb)
# Modified by Mika (MT)
# Modified by Microsoft Copilot (MC)

### History ###
# 2025-12-15 Mika: Removed unnecessary 0.0001 s addition to start time now that enforces a half‑open window and guards against boundary overlap.
# 2025-12-15 Copilot: strict half-open + tolerance gap detection; improved messages.
# 2025-12-15 Copilot: Bundled per request window (not only hourly), keep micro-segments.
# 2025-12-15 Copilot: Duration-aware logs; per-segment sampling rate; per-channel tolerance.
# 2025-12-15 Copilot: EarthScope-aligned gap classification (Δt > 1.5×period).
# 2025-12-15 Copilot: **Explicit JITTER vs GAP messages** (per-channel tolerance for jitter).
# 2025-12-15 Mika: Removed restriction that only allowed data requests up to midnight. Reduced trunctime from 24 hours to 60 seconds in run_prest.txt to allow for delay in data ingestion to the OOI database (CI).
# 2025-10-01 Mika: This message in log files can be ignored: <frozen importlib._bootstrap>:241: RuntimeWarning: numpy.ndarray size changed, may indicate binary incompatibility.
# 2025-09-23 Mika: Reformatted script for Python 3.10.
...
# Adding 0.0001 s to startTime is no longer necessary. Code now enforces a half‑open window [start, end) and guards against boundary overlap. MT
startTime = str(file_end_time) 
...
# difTime = UTCDateTime(endTime) - UTCDateTime(startTime) --> I don't think the difTime comparison is needed. MT.

# Truncation control (Now - trunctime) or fixed data_endtime
# Introduce a delay (e.g., trunctime in run_prest.txt) in data requests to allow for data ingestion time into the OOI database. MT
...
# if UTCDateTime(endTime) <= UTCDateTime(truncTime) and difTime >= float(run["diftime"][0]): --> I don't think the difTime comparison is needed. MT.
if UTCDateTime(endTime) <= UTCDateTime(truncTime):
    ...
    # If data request fails, email is sent to operator, data gap is logged, and update endtime_...txt is advanced by time_interval, and program exits.
    ...
    if complete == "complete":
        ...
        # MC: global period estimate for classification; round only for logging
        sp_unrounded_global = float(np.median(np.diff(t))) # seconds/sample, updated to remove default 1 s sampling on 12/22/2025. MT
        sp_log = round(sp_unrounded_global, 6)
        print(f"Derived global sampling period: {sp_log:.6f} s")
        ...
        # --- EarthScope-aligned gap classification (global) --- # MC
        dt_all = np.diff(new_t)
        gap_threshold_global = 1.5 * sp_unrounded_global  # MUSTANG rule
        gap_idx = np.where(dt_all > gap_threshold_global)[0]
        for gi in gap_idx:
            prev_ts = UTCDateTime(str(utcdata1900(float(new_t[gi]))))
            next_ts = UTCDateTime(str(utcdata1900(float(new_t[gi+1]))))
            delta = float(dt_all[gi])            
            print(
                f"GAP: {prev_ts} → {next_ts} "
                f"Δt={delta:.6f}s > 1.5×sp({gap_threshold_global:.6f}s)"
            )

            if gap_enabled:
                with open(os.path.join(runPath, gap_file_name), "a") as output:
                    output.write(
                        f"GAP: {prev_ts} {next_ts} Δt={delta:.6f}s > 1.5×sp({gap_threshold_global:.6f}s)\n"
                    )

        # Split into contiguous segments at gap positions (split before element gi+1)
        split_idx = (gap_idx + 1).tolist()
        new_utc_str = [str(x) for x in new_utc]
        new_utc_arr = np.array(new_utc_str, dtype=object)
        data_split = np.split(new_utc_arr, split_idx)

        # Informational start-boundary check
        if (str(file_end_time)[11:27] != "23:59:59.999999Z" and file_end_time + sp_unrounded_global != UTCDateTime(data_split[0][0])):
            ...
            if delta_start < sp_unrounded_global:
                print("Start-boundary adjustment: first sample " + str(UTCDateTime(data_split[0][0])) +
                      f" occurs {delta_start:.6f}s after expected {file_end_time + sp_unrounded_global}. Dropping boundary (last) sample to avoid overlap.") # Logging only, actual drop occurs later via clamping. MT
            else:
                print("Start-of-window gap: first sample " + str(UTCDateTime(data_split[0][0])) +
                      f" arrives {delta_start:.6f}s after expected {file_end_time + sp_unrounded_global} (> sp). Splitting segments.")

        last_written_end_dt = None  # MC: track last clamped end across channels/segments so timestamp for next data request is set correctly in endtime_ file. MT
        ...
        for k in range(len(channel_ls)):
            ...
            # MC: per-channel tolerance configuration (for JITTER logging)
            try:
                gap_tol_factor = float(channel_param["GAP_TOL_FACTOR"][0])
                tol_seconds_cfg = float(channel_param["TOL_SECONDS"][0])
                min_tol_seconds = float(channel_param["MIN_TOL_SECONDS"][0])
                min_segment_seconds = float(channel_param["MIN_SEGMENT_SECONDS"][0])
            except (KeyError, IndexError, ValueError) as e:
                raise ValueError(f"Missing or invalid channel_param value: {e}")
            
            # --- NEW: Relative jitter tolerance with an optional absolute floor --- MC
            # Use Jitter_Tol_Pct if present; otherwise fall back to Snap_Tol (e.g., 0.005 = 0.5%)
            try:
                tol_pct = float(channel_param.get("Jitter_Tol_Pct", [channel_param.get("Snap_Tol", [0.005])[0]])[0])
            except Exception:
                tol_pct = 0.005  # default 0.5%

            # Absolute floor (seconds) to prevent the near-period band from being too tiny at fast rates
            abs_floor = float(channel_param.get("Jitter_Tol_Min_Seconds", [0.0])[0])

            # If you still want an absolute tolerance path available, compute it here (kept for potential fallback)
            if tol_seconds_cfg > 0:
                tol_seg = max(tol_seconds_cfg, min_tol_seconds)  # absolute seconds tolerance
            else:
                tol_seg = max(gap_tol_factor * 1.0, min_tol_seconds)  # keeps your legacy behavior available
            ...
            for s in range(len(data_split)):
                ...
                # Keep only timestamps <= clamped end (guard windows shorter than one period)
                ti_dt = [UTCDateTime(ts) for ts in ti]
                n_keep = bisect.bisect_right(ti_dt, cur_end_dt) # n_keep indicates location where cur_end_dt would be inserted in ti_dt. 
                if n_keep == 0:
                    print(f"Segment from {ti_dt[0]} has no samples <= {cur_end_dt}, skipping.")
                    continue

                # Per-segment sampling period/rate from timestamps
                if n_keep >= 3:
                    ...
                    sp_seg = sp_unrounded_global
                sp_rate_seg = 1.0 / sp_seg
                
                # Use nominal sample rate for instrument unless snap tolerance exceeded. MT
                nominal_rate = float(channel_param["C_sample_rate"][0])  # from channel param file
                snap_tol = float(channel_param["Snap_Tol"][0])  # 0.5% default
                if abs((1/sp_seg) - nominal_rate)/nominal_rate <= snap_tol:  # 0.5% tol
                    print(f"Sampling rate snapped: observed {sp_rate_seg:.6f} Hz → nominal {nominal_rate:.6f} Hz "
                        f"(tol {snap_tol*100:.2f}%, channel {channel_ls[k]})")
                    sp_rate_seg = nominal_rate
                else:
                    sp_rate_seg = 1.0 / sp_seg
 
                # Per-channel tolerance (for JITTER logging)
                if tol_seconds_cfg > 0:
                    tol_seg = max(tol_seconds_cfg, min_tol_seconds)
                else:
                    tol_seg = max(gap_tol_factor * sp_seg, min_tol_seconds)

                # --- JITTER logging (per-channel) ---  # relative tolerance + near-period band, MC
                if n_keep >= 2:
                    diffs_local = np.diff([ts.timestamp for ts in ti_dt[:n_keep]])
                    seg_gap_threshold = 1.5 * sp_seg

                    # Near-period band halfwidth: relative (tol_pct * sp_seg) with optional absolute floor
                    band_halfwidth = max(tol_pct * sp_seg, abs_floor)
                    lower_bound = max(sp_seg - band_halfwidth, 0.0)
                    upper_bound = min(sp_seg + band_halfwidth, seg_gap_threshold)

                    for jj, dval in enumerate(diffs_local):
                        prev_j = ti_dt[jj]
                        next_j = ti_dt[jj + 1]

                        # Relative deviation from nominal period (unitless)
                        dev_rel = abs(dval / sp_seg - 1.0)

                        # Require intervals to be near the nominal period; exclude micro-bursts or overlaps
                        is_near_period = (dval >= lower_bound) and (dval <= upper_bound)

                        # JITTER only if: near-period, below gap threshold, and relative deviation exceeds tol_pct
                        if is_near_period and (dval < seg_gap_threshold) and (dev_rel > tol_pct):
                            print(
                                f"JITTER[{channel_ls[k]}]: {prev_j} → {next_j} "
                                f"Δt={dval:.6f}s (dev_rel={dev_rel*100:.3f}%) "
                                f"> tol({tol_pct*100:.3f}%) and ≤ 1.5×sp({seg_gap_threshold:.6f}s)"
                            )
                            if gap_enabled:
                                with open(os.path.join(runPath, gap_file_name), "a") as output:
                                    output.write(
                ...
                if min_segment_seconds > 0:
                    ...
                    if seg_duration < min_segment_seconds:
                        print(f"Micro-segment kept (duration {seg_duration:.3f}s < {min_segment_seconds}s): "
            ...
            if len(interval_stream) > 0:
                ...
                window_seconds = float(UTCDateTime(endTime) - UTCDateTime(startTime))
                expected_n = int(round(window_seconds * sp_rate_first))
                actual_n = int(sum(tr.stats.npts for tr in interval_stream))
                print(f"[Expected ~{expected_n} samples over {window_seconds:.1f}s at {sp_rate_first:.6f} Hz, wrote {actual_n}")
                ...
                print(f"[No segments to write for channel {channel_ls[k]} in this window.")

        # After all channels, write endtime_<...>.txt
        if last_written_end_dt is not None:
            with open(os.path.join(runPath, endtime_file_name), "w") as output:
                output.write(str(last_written_end_dt))
            print(endtime_file_name, "set =", str(last_written_end_dt), "[bundled]")
        else:
            # No data segments; advance by interval to avoid stalling
            with open(os.path.join(runPath, endtime_file_name), "w") as output:
                output.write(str(UTCDateTime(startTime) + float(run["time_interval"][0])))
            print(endtime_file_name, "set =", str(UTCDateTime(startTime) + float(run["time_interval"][0])), "[no data]")

# I don't think the difTime comparison is needed. MT.
