...
# 2025-12-12 Copilot: **EarthScope-aligned gap classification** (Δt > 1.5×period) for splitting (# MC).
...
if UTCDateTime(endTime) <= UTCDateTime(truncTime) and difTime >= float(run["diftime"][0]):
    ...
    if complete == "complete":
        ...
        # --- EarthScope-aligned gap classification --- # MC
        dt_all = np.diff(new_t)
        gap_threshold = 1.5 * sp_unrounded_global  # EarthScope MUSTANG rule: gap if Δt > 1.5×period
        gap_idx = np.where(dt_all > gap_threshold)[0]  # indices where gap occurs between i and i+1
        # Log detected gaps
        for gi in gap_idx:
            ...
            print(f"[MC] EarthScope GAP: Δt={delta:.6f}s > 1.5×sp ({gap_threshold:.6f}s) between {prev_ts} and {next_ts}")
            if int(run.get("gap", [0])[0]) == 1:
                with open(os.path.join(runPath, gap_file_name), "a") as output:
                    output.write(f"EarthScope GAP: {prev_ts} {next_ts} Δ={delta:.6f}s > 1.5×sp\n")
        ...
        for k in range(len(channel_ls)):
            ...
            # MC: per-channel tolerance configuration (for jitter logging, not splitting)
            gap_tol_factor   = float(channel_param.get("GAP_TOL_FACTOR", ["0.20"])[0])  # fraction of period
            tol_seconds_cfg  = float(channel_param.get("TOL_SECONDS",    ["-1"])[0])    # -1 => unused
            min_tol_seconds  = float(channel_param.get("MIN_TOL_SECONDS", ["0.005"])[0]) # 5 ms floor
            ...
            for s in range(len(data_split)):
                ...
                # Per-channel tolerance (for jitter logging only)
