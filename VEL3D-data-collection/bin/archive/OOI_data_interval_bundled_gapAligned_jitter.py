...
# 2025-12-12 Copilot: EarthScope-aligned gap classification (Δt > 1.5×period) (# MC).
# 2025-12-12 Copilot: **Explicit JITTER vs GAP messages** (per-channel tolerance for jitter) (# MC).
...
if UTCDateTime(endTime) <= UTCDateTime(truncTime) and difTime >= float(run["diftime"][0]):
    ...
    if complete == "complete":
        ...
        sp_unrounded_global = float(np.median(np.diff(t))) if len(t) >= 3 else 1.0  # seconds/sample
        ...
        for gi in gap_idx:
            ...
            print(f"[MC] GAP: Δt={delta:.6f}s > 1.5×sp ({gap_threshold_global:.6f}s) {prev_ts} → {next_ts}")
            if gap_enabled:
                with open(os.path.join(runPath, gap_file_name), "a") as output:
                    output.write(f"GAP: {prev_ts} {next_ts} Δ={delta:.6f}s > 1.5×sp\n")
        ...
        for k in range(len(channel_ls)):
            ...
            gap_tol_factor   = float(channel_param.get("GAP_TOL_FACTOR", ["0.20"])[0])
            tol_seconds_cfg  = float(channel_param.get("TOL_SECONDS",    ["-1"])[0])
            min_tol_seconds  = float(channel_param.get("MIN_TOL_SECONDS", ["0.005"])[0])
            ...
            for s in range(len(data_split)):
                ...
                # --- JITTER logging (per-channel) --- # MC
                # Iterate the diffs again to classify jitter events within this contiguous segment
                if n_keep >= 2:
                    diffs_local = np.diff([ts.timestamp for ts in ti_dt[:n_keep]])
                    seg_gap_threshold = 1.5 * sp_seg
                    for jj, dval in enumerate(diffs_local):
                        if dval > tol_seg and dval <= seg_gap_threshold:
                            prev_j = ti_dt[jj]
                            next_j = ti_dt[jj+1]
                            print(f"[MC] JITTER[{channel_ls[k]}]: Δt={dval:.6f}s > tol({tol_seg:.6f}s) and ≤ 1.5×sp({seg_gap_threshold:.6f}s) {prev_j} → {next_j}")
                            if gap_enabled:
                                with open(os.path.join(runPath, gap_file_name), "a") as output:
                                    output.write(f"JITTER[{channel_ls[k]}]: {prev_j} {next_j} Δ={dval:.6f}s > tol({tol_seg:.6f}s) ≤ 1.5×sp\n")
