...
# 2025-12-12 Copilot: Implemented Option A (strict half-open windows), clarified gap messages, and marked changes with # MC.
...
if UTCDateTime(endTime) <= UTCDateTime(truncTime) and difTime >= float(
    ...
    if complete == "complete":
        ...
        # Convert HTTPServer URL to OPENDAP URL (per run params). MT
        ...
        try:
            ...
            # netCDF4.Dataset returns time as raw numeric values (a NumPy masked array) stored in the file—typically “X units since Y-epoch” (e.g., seconds since 1900-01-01), with masking based on _FillValue.
            ...
            t = fh.variables[str(run["datatime"][0])][:]  # datatime = time; netCDF4.Dataset returns time as raw numeric values (numpy masked array). These are converted to UTCDateTimes at line ~496. MT
        ...
        if len(t) >= int(run["data_sp"][0]):
            ...
            sp_unrounded = float(np.median(np.diff(t)))  # MC
            sp = round(sp_unrounded, 6)  # MC: log-friendly precision
            print('Sampling interval (median):', sp)  # MC
        ...
        sp_rate = 1.0 / sp_unrounded  # MC: arithmetic uses unrounded step
        # MC: tolerance for gap detection (25% of nominal interval, min 1 ms)
        tol = max(0.25 * sp_unrounded, 0.001)  # MC
        ...
        # Minimum samples accepted (data_sp) ,mb
        ...
        sp_r = round(float(sp_unrounded), rd)  # MC: ensure same rounding on reference spacing
        tol_r = round(float(tol), rd)  # MC
        ...
        for p in range(len(new_t)):
            ...
            if p == 0:
                ...
                gap_val = round(float(dt[p - 1]), rd)
                gap = abs(gap_val - sp_r) > tol_r  # MC: tolerance-based gap detection
                statement.append(gap)
                if gap:
                    print('Gap detected: spacing', gap_val, 's (expected', sp_r, 's, tol', tol_r, 's)')  # MC clearer
        ...
        # Start-boundary adjustments / messages
        # 5* and 6* conditions clarified with explicit deltas. MC
        if (
            str(file_end_time)[11:27] != "23:59:59.999999Z"
            and file_end_time + sp_unrounded != UTCDateTime(data_split[0][0])
        ):
            delta_start = (UTCDateTime(data_split[0][0]) - (file_end_time + sp_unrounded))
            if delta_start < sp_unrounded:
                print(
                    # MC: clearer message
                    "5* [MC] Start-boundary adjustment: first sample "
                    + str(UTCDateTime(data_split[0][0]))
                    + " occurs "
                    + f"{delta_start:.6f}s "
                    + "after expected boundary "
                    + str(file_end_time + sp_unrounded)
                    + ". Dropping boundary sample to avoid overlap."
                )
                if int(run["gap"][0]) == 1:
                    with open(os.path.join(runPath, gap_file_name), "a") as output:
                        output.write(
                            "%s %s %s\n"
                            % (
                                "5* [MC] Start-boundary adjustment:",
                                str(file_end_time + sp_unrounded),
                                str(data_split[0][0]),
                            )
                        )
                ...
                print(
                    "6* [MC] Start-of-window gap: first sample "
                    + str(UTCDateTime(data_split[0][0]))
                    + " arrives "
                    + f"{delta_start:.6f}s "
                    + "after expected "
                    + str(file_end_time + sp_unrounded)
                    + " (> sp). Splitting segments."
                )
                if int(run["gap"][0]) == 1:
                    with open(os.path.join(runPath, gap_file_name), "a") as output:
                        output.write(
            ...
            delta_start = (UTCDateTime(data_split[0][0]) - requested_start)
            if delta_start < sp_unrounded:
                print(
                    "7 [MC] Requested-start alignment: first sample "
                    + str(UTCDateTime(data_split[0][0]))
                    + " occurs "
                    + f"{delta_start:.6f}s "
                    + "after requested_start "
                    + str(requested_start)
                    + " (< sp). Adjusting to avoid overlap."
                )
                if int(run["gap"][0]) == 1:
                    with open(os.path.join(runPath, gap_file_name), "a") as output:
                        output.write(
                            "%s %s %s\n"
                            % (
                                "7 [MC] Requested-start alignment:",
                                startTime,
                                data_split[0][0],
                            )
                        )
                ...
                print(
                    "8* [MC] Requested-start gap: first sample "
                    + str(UTCDateTime(data_split[0][0]))
                    + " occurs "
                    + f"{delta_start:.6f}s "
                    + "after requested_start "
                    + str(requested_start)
                    + " (>= sp). Treating as gap and splitting."
                )
                if int(run["gap"][0]) == 1:
                    with open(os.path.join(runPath, gap_file_name), "a") as output:
                        output.write(
                            "%s %s %s\n"
                            % (
                                "8* [MC] Requested-start gap:",
                                startTime,
                                str(UTCDateTime(data_split[0][0])),
                            )
                        )

        # Write endtime for single-segment case
        if len(data_split) == 1:
            # MC: half-open clamp for single segment — end-exclusive by one sample
            cur_end_dt = UTCDateTime(data_split[0][-1])
            cur_end_dt = min(cur_end_dt, requested_end - sp_unrounded)  # MC
            with open(os.path.join(runPath, endtime_file_name), "w") as output:
                output.write(str(cur_end_dt))
            print(endtime_file_name, "set =", str(cur_end_dt), "(half-open clamp) [MC]")
        else:
            ...
            for j in range(len(data_split)):
                ...
                if j == len(data_split) - 1:
                    ...
                    if len(ti) >= int(run["data_sp"][0]):
                        ...
                        print(
                            start_time + " " + end_time + ": ",
                            "9 - Segment too short (" + str(len(ti)) + ") < " + run["data_sp"][0] + ". Skipping final data split.",
                        )  # MC clearer
                        if int(run["gap"][0]) == 1:
                            with open(
                                ...
                                output.write(
                                    "%s %s %s\n"
                                    % (
                                        "9 [MC] Final data split too short. Skipping:",
                                        start_time,
                                        end_time,
                                    )
                                )
                    ...
                    if len(ti) >= int(run["data_sp"][0]):
                        ...
                        if int(run["gap"][0]) == 1:
                            delta_mid = (
                                UTCDateTime(start_time2) - UTCDateTime(end_time)
                            )
                            with open(
                                os.path.join(runPath, gap_file_name), "a"
                            ) as output:
                        ...
                        print(
                            start_time + " " + end_time + ": ",
                            "Segment too short (" + str(len(ti)) + ") < " + run["data_sp"][0] + ". Skipping.",
                        )  # MC clearer
                        if int(run["gap"][0]) == 1:
                            with open(
                                os.path.join(runPath, gap_file_name), "a"
                                ...
                                output.write(
                                    "%s %s %s\n"
                                    % (
                                        "11* [MC] Segment too short. Skipping:",
                                        start_time,
                                        end_time,
                                    )
                                )
                            # Also log the raw times around the gap
                            with open(
                                os.path.join(runPath, gap_file_name), "a"
                            ) as output:
                                output.write(
        ...
        # --- MiniSEED writing with strict half-open windows --- # MC
        ...
        for k in range(len(channel_ls)):
            ...
            l_idx = start  # Actual start index in the netCDF arrays
            ...
            for s in range(len(data_split)):
                ...
                if len(ti) >= int(run["data_sp"][0]):
                    # MC: enforce half-open: end-exclusive by one sample
                    cur_start_dt = UTCDateTime(start_time_str)  # MC
                    cur_end_dt   = UTCDateTime(end_time_str)    # MC
                    if s < len(data_split) - 1:
                        next_start_dt = UTCDateTime(data_split[s + 1][0])  # MC
                        cur_end_dt = min(cur_end_dt, next_start_dt - sp_unrounded)  # MC
                    else:
                        cur_end_dt = min(cur_end_dt, requested_end - sp_unrounded)  # MC

                    # MC: determine number of samples to keep based on timestamps <= cur_end_dt
                    ...
                    for idx, ts_dt in enumerate(ti_dt):
                        if ts_dt <= cur_end_dt:
                            ...
                            break
                    n_keep = end_keep_idx + 1  # inclusive count from start

                    # Slice data to n_keep samples
                    ...
                    # MC: set accurate start time using UTCDateTime
                    stats["starttime"] = cur_start_dt  # MC

                    st = Stream([Trace(data=data, header=stats)])
                    encoding_name = str(st[0].data.dtype).upper()

                    # MC: compute filename end using clamped cur_end_dt
                    end_time_for_name = str(cur_end_dt)  # MC
                    ...
                    st.write(
                        ...
                        + "."
                        + "{:0>3}".format(
                            str(time.strptime(start_time_str[0:10], "%Y-%m-%d").tm_yday)
                        )
                        + "."
                        + start_time_str[11:23].replace(":", ".")
                        + "-"
                        + end_time_for_name[0:4]
                        + "."
                        + "{:0>3}".format(
                            str(time.strptime(end_time_for_name[0:10], "%Y-%m-%d").tm_yday)
                        )
                        + "."
                        + end_time_for_name[11:23].replace(":", ".")
                        + mseed_file_ext,
                        format="MSEED",
                    ...
                    # Advance l_idx by the number of samples written
                    l_idx = l_idx + n_keep  # MC

                elif len(ti) < int(run["data_sp"][0]):
                    # Skip short segment
                    print(
                        "[MC] Segment skipped: only",
                        len(ti),
                        "samples (<",
                        run["data_sp"][0],
                        ")",
