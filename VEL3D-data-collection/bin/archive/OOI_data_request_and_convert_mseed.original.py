...
try:
    ...
    if UTCDateTime(end_time) <= UTCDateTime(trunc_time):
        ...
        if 'prest' in run_name:
            stream_tag = "streamed/" + run_name + "_real_time?include_provenance=true&format=application/netcdf" # Absolute pressure sensor
        elif 'lily' in run_name or 'nano' in run_name:
            stream_tag = "streamed/botpt_" + run_name + "_sample?include_provenance=true&format=application/netcdf" # Bottom Pressure and Tilt
        ...
        if complete == "complete":
            ...
            if npts < 2:
                ...
                # Step 4: Estimate how many points we *should* have in this window
                ...
                # Step 5: Identify intervals larger than the chosen threshold
            ...
            for k in range(len(channel_ls)):
                ...
                for s in range(len(data_split)):
                    ...
                    # Set current time
                    stats["starttime"] = start_time_splt # start time of the data_split segment
