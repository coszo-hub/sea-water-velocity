...
# 2026-03-23 Mike: Updated the gap detection to be more robust. 
    ...
    if UTCDateTime(end_time) <= UTCDateTime(trunc_time):
        ...
        if complete == "complete":
            ...
            if npts < 2:
                ...
                #
                # A simple median(Δt) can fail when:
                #   - Data are sparse or thinned
                #   - Large gaps dominate the distribution
                #   - Short bursts of closely spaced samples skew statistics
                #
                # If the sampling period is mis-estimated:
                #   - Normal data may be flagged as gaps
                #   - Gap sizes will be miscalculated
                #
                # Therefore, we estimate the sampling period from the *shape*
                # of the Δt distribution, not from a single statistic.
                ...
                if dt.size == 0:
                    ...
                    # If no valid Δt values remain, we cannot estimate a sampling period.
                    # This can occur with extremely sparse or malformed data.
                    #
                    # We set placeholders so downstream logic does not crash,
                    # but gap detection will effectively be skipped.
                    ...
                    # We apply two lightweight filters to remove extreme outliers
                    # while preserving the dominant sampling cadence:
                    #
                    #   1) IQR filter → removes extreme tails
                    #   2) Percentile trim → handles skewed distributions
                    ...
                    if hist.sum() == 0:
                        ...
                        # If no single Δt dominates the distribution, the data likely
                        # contains multiple sampling rates (e.g., redeployment or
                        # configuration change). We alert the operator. 
                ...
                # --- 4) Email alert if calculated period deviates from the deployment’s nominal (hybrid threshold) ---
